...
public class KThread {
    ...
    private static class ConTest implements Runnable {
		...
		public void run() {
			testLock.acquire();
			System.out.println("*** thread " + which + " start!!");
			if(which % 2 == 1)
				testCon.sleep();
			else
				testCon.wake();
			System.out.println("*** thread " + which + " finished!!");
			testLock.release();
		}

		private int which;
		private static Lock testLock = new Lock();
		private static Condition2 testCon = new Condition2(testLock);
    }

    private static class AlarmTest implements Runnable {
		AlarmTest(int which, int time) {
			this.time = time;
			this.which = which;
		}
	
		public void run() {
			System.out.println("*** Thread " + which + " Sleep " + time + " start!!");
			testAlarm.waitUntil(time);
			System.out.println("*** Thread " + which + " Sleep " + time + " finished!!");
		}
	...
	KThread c = new KThread(new ConTest(3)).setName("forked thread");
	KThread d = new KThread(new ConTest(4)).setName("forked thread");
	KThread e = new KThread(new ConTest(5)).setName("forked thread");
	KThread f = new KThread(new ConTest(6)).setName("forked thread");
	f.fork();
	a.fork();
	c.fork();
	e.fork();
	...
	new KThread(new AlarmTest(3, 3500)).setName("forked thread").fork();
